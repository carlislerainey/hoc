...
fit <- stan("stan/het-t.stan", 
            data = stan_data_list, 
            iter = 1000,
            thin = 4,
            chains = 4, 
            seed = 8792)
write_rds(fit, "sim/gen/stanfit-het-t.rds")

# evalute stan model
log_lik0 <- extract_log_lik(fit) # see ?extract_log_lik
loo0 <- loo(log_lik0)
write_rds(loo0, path = "sim/gen/loo-het-t.rds")
...
ppc_dens_overlay(y, y_rep[1:20, ]) +
  labs(title = "Posterior Predictive Distribution (Simulated Data)",
       subtitle = "Heteroskedastic Student's t Regression")
ggsave("sim/gen/ppd-het-t.png", height = 3, width = 4, scale = 1.3)
